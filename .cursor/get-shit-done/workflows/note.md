...
- **Global scope**: `/home/ivan/repos/tunnel/.cursor/notes/{YYYY-MM-DD}-{slug}.md` — fallback when no `.planning/`, or when `--global` flag is present
...
2. Ensure the notes directory exists (`.planning/notes/` or `/home/ivan/repos/tunnel/.cursor/notes/`)
...
2. Glob `/home/ivan/repos/tunnel/.cursor/notes/*.md` (if directory exists) — global notes
...
Global (/home/ivan/repos/tunnel/.cursor/notes/):
...
2. **No `.planning/`**: Falls back to global `/home/ivan/repos/tunnel/.cursor/notes/` — works in any directory
