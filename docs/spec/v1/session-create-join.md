...
保留：`0x00` 不得作为有效控制消息类型（可与实现约定为「未初始化」探测，不在本规范强制）；`0x05` 及以上预留给后续阶段或扩展。
...
错误应答与关闭原因在 Phase 5 错误码目录中统一（例如未知 session、拒绝加入）；本阶段实现可返回带 **错误码** 的控制消息帧，具体 opcode 待 ERR 规范绑定。
