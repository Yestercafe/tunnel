...
| **Client** | 与 relay 建立传输（WS/WSS）、加入 session、发送/接收帧 | 可运行在浏览器或 Go |
...
1. Client A 与 Relay 建立 **TLS（边缘终止）** 下的连接（如 WSS）。  
...
- **传输层**：WebSocket 或未来 TCP；只保证字节双向可达。  
