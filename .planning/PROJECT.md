...
- **进行中：** **v1.1** — Phase 7–9 已完成（`pkg/protocol`、`pkg/client`、`pkg/relay` 控制面、`cmd/tunnel`）；继续 **Relay 数据面（Phase 10）/ E2E**。  
...
- [x] **RELAY-IMPL（控制面）：** 最小 Relay：TCP 监听 + TLS，解析 v1 帧，维护 session ↔ peers，处理 SESSION_CREATE / JOIN 控制面 — Phase 9
...
*Last updated: 2026-04-04 — Phase 9（pkg/relay 控制面）已完成*
